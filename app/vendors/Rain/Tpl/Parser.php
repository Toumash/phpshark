...
class Parser
{
	...
	/**
	 * Check if the templates exist and compile it if necessary
	 *
	 * @param string $template : name of the file of the templates
	 *
	...
	protected function checkTemplate($template)
	{
		...
		// Make directories to array for multiple templates directory
	 ...
	 *
	 * @param string $string : RainTpl templates string to compile
	 *
	 * @return string: full filepath that php must use to include
	 */
	 ...
	 * @param string $parsedTemplateFilepath : cache file where to save the templates
	 */
	...
	protected function compileTemplate($code, $isString, $templateBasedir, $templateDirectory, $templateFilepath)
	{
		...
		if ($codeSplit) //read all parsed code
		{
			foreach ($codeSplit as $html) {
				...
				elseif (preg_match($tagMatch['include'], $html, $matches)) {
					...
					//get the folder of the actual templates
		...
		if ($isString) {
			...
			if ($openIf > 0) {
				$e = new SyntaxException("Error! You need to close an {if} tag in $templateFilepath templates");
				throw $e->templateFile($templateFilepath);
			}

			if ($loopLevel > 0) {
				$e = new SyntaxException("Error! You need to close the {loop} tag in $templateFilepath templates");
				throw $e->templateFile($templateFilepath);
			}
		}
	...
	protected function blackList($html)
	{
		...
		if (preg_match($this->config['black_list_preg'], $html, $match)) {
			...
			$e = new SyntaxException('Syntax ' . $match[0] . ' not allowed in templates: ' . $this->templateInfo['template_filepath'] . ' at line ' . $line);
